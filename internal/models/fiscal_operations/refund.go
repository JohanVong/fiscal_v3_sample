...
func (self RefundReqest) Valid() (err error) {
...
func (self RefundReqest) Create() (doc models.Document, positions []models.Position, err error) {
	var (
		pos          models.Position
		address      models.Address
		section      models.Section
		cash_balance models.Balance
	)
	...
	doc.IdTypedocument = OPERATION_REFUND
	...
	if self.Cash.Cmp(new(decimal.Big)) > 0 {
		...
		err = tx.Where("idKKM = ? AND idTypeBalance = ?", self.IdKkm, 1).First(&cash_balance).Error
		if err != nil {
			return
		}
		current_cash_balance, _ := cash_balance.Amount.Float64()
		sum_f, _ := sum_to_balance.Float64()
		if current_cash_balance-sum_f < 0 {
			err = errors.New("Сумма операции превышает баланс кассы")
			return
		}
